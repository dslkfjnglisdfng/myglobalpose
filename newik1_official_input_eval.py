...
from newik1_official_input_train import MODEL_TYPE, build_model
from pl_curve_eval import build_pl_curve, selected_imu_fields
from test import MotionEvaluator


def build_official_ik1(checkpoint_path):
...
def evaluate(records, pl_curve, ik1_model=None, max_eval_sequences=0, imu_input_mode='processed'):
    ...
    for record in selected:
        ...
        rows.append({
            ...
            'ik1_residual_norm_mean': 0.0,
            'ik1_residual_norm_max': 0.0,
...
def main():
    parser = argparse.ArgumentParser(description='Evaluate official-input NewIK1_v1 inside streaming GPNet.')
    ...
    result = {
        ...
        'pl_backend': 'curve_v1',
        'ik1_backend': 'official_input_v1' if args.ik1_checkpoint else 'original',
    ...
    try:
        ...
        ik1_model, ik1_config = (None, None)
        if args.ik1_checkpoint:
            ik1_model, ik1_config = build_official_ik1(args.ik1_checkpoint)
        rows = evaluate(
            records,
            pl_curve=pl_curve,
            ik1_model=ik1_model,
