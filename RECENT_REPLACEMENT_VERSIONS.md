...
| `PL-s1 / joint-leaf acceleration` | `newpl_joint_leaf_acc_20260619` | formal module-only run complete; not effective, both acceleration modes slightly worsen p_leaf L2 |
...
| newpl_joint_leaf_acc_20260619 | PL-s1 module route only | switch NewPL cache/eval target from vertex PL to SMPL joint-leaf positions, with optional frozen leaf-acc predictor features | three modes: `baseline_jointtarget_84D=aRB+wRB+RRB+gR0`, `acc_root_102D=84D+a_smoothed_R+a_output_R`, `acc_mixed_102D=84D+a_smoothed_W+a_output_W` | 18D state is still `15+3`, but legacy loss key `pRB` now means `p_leaf_joint_R`; old IK/full-pipeline consumers are intentionally not used | no new loss; original `pl_curve_loss`, derivative-aware target controls, init36, spline decoder, and `control_physical` selection | full cache/train/eval complete: AMASS 80 with early stop, DIP 40 with early stop; module-only DIP test and TC test | no; after DIP FT, DIP p_leaf L2 baseline `5.702842`, acc_root `5.703787`, acc_mixed `5.703366`; TC baseline `5.174598`, acc_root `5.176143`, acc_mixed `5.176165`; gR1 gains are only `0.000014-0.000170 deg` | not applicable; no IK/full-pipeline by contract | rejected/diagnostic; acceleration features do not lower p_leaf_joint_R L2 on same cache |
