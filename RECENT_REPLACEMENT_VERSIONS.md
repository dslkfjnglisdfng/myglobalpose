...
| `PL-s1 / joint-leaf acceleration` | `newpl_joint_leaf_acc_20260619`, `pl_joint_control_acc_aug102_v1` | `pl_joint_control_acc_aug102_v1` is the current joint-target smoke: target is SMPL joint pRB, input is legacy84 + frozen joint-acc R15 + smoothed root-acc R3; no full result yet |
...
| pl_joint_control_acc_aug102_v1 | PL-s1 module route only | new joint-target control experiment with frozen joint acceleration auxiliary input; this is not a continuation of legacy vertex PL target | `frozen_joint_acc_aug102 = aRB[18]+wRB[18]+RRB[45]+gR0[3]+frozen_joint_acc_R[15]+root_acc_smooth_R[3]`; explicit feature_mode prevents confusion with older smooth_residual 102D layout | 18D target is `joint_pRB[15]+gR[3]`, where `joint_pRB=(SMPL joints [18,19,4,5,15]-root joint) @ root_R`; legacy vertex pRB is diagnostic only | joint-control loss weights: pRB 1.0, gR1 0.3, gt_control_pRB 0.5, gt_control_gR1 0.1, pRB_dot 0.5, pRB_ddot 0.1, pRB_ddot_smooth/gR_smooth/control_point_prior/tail_update_prior 0.001 | smoke only: DIP-val max_sequences=1, max_frames=120, 1 epoch, hidden 128; frozen `imu_leaf_acc_predictor_v1` loaded eval-only with checkpoint normalization | smoke passed: feature_dim 102, target_dim 18, joint-vs-vertex diagnostic L2 0.261554 m, frozen_joint_acc_R norm 0.123468, root_acc_smooth_R norm 0.063012 | no full S4/full-pipeline result; module smoke eval reports joint_pos 0.267529 m, joint_vel 0.022015 m/s, joint_acc 1.920843 m/s^2, gravity 0.582246 deg | keep as current smoke implementation; full AMASS -> DIP training is next action if approved |
