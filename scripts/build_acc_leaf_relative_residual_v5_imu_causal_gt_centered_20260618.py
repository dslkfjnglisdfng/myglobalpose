#!/usr/bin/env python3
"""Build asymmetric leaf-relative acceleration residual audit caches.
...
The realtime IMU smoother is a zero-lookahead causal Butterworth low-pass filter.
The GT side uses a centered moving average target-only smoother.
Both are applied only to the centered-difference valid segment so NaN boundary
frames from GT finite differences never enter the filters.
"""
...
DEFAULT_OUTPUT_ROOT = Path("data/experiments/acc_leaf_relative_residual_v5_imu_causal_gt_centered_20260618")
ROOT_INDEX = 5
LEAF_INDICES = (0, 1, 2, 3, 4)
BUTTER_ORDER = 2
BUTTER_CUTOFF_HZ = 4.0
GT_MA_WINDOW = 9
...
def causal_butterworth_valid_segment(x: torch.Tensor, valid: torch.Tensor, args: argparse.Namespace) -> torch.Tensor:
    ...
    if not torch.isfinite(segment).all():
        raise ValueError("causal Butterworth segment contains non-finite values")
    out[start:end] = causal_butterworth_lowpass_sequence(
        segment,
        fs=float(args.butter_fps),
        cutoff_hz=float(args.butter_cutoff_hz),
        order=int(args.butter_order),
    )
    return out


def centered_ma_valid_segment(x: torch.Tensor, valid: torch.Tensor, window: int = GT_MA_WINDOW) -> torch.Tensor:
    out = torch.full_like(x, float("nan"))
    valid_idx = valid.nonzero(as_tuple=False).flatten()
    if valid_idx.numel() == 0:
        return out
    ...
    segment = x[start:end]
    if not torch.isfinite(segment).all():
        raise ValueError("centered moving average segment contains non-finite values")
    out[start:end] = smooth_centered(segment, window=window, mode="centered_moving_average")
    return out
...
def build_record(
    ...
    aIMU_leaf_rel_butter = causal_butterworth_valid_segment(aIMU_leaf_rel_raw, valid, args)
    aGT_leaf_rel_centered = centered_ma_valid_segment(aGT_leaf_rel_raw, valid, GT_MA_WINDOW)
    aGT_leaf_rel_butter = causal_butterworth_valid_segment(aGT_leaf_rel_raw, valid, args)
    meta = {
        ...
        "imu_smoother": {
            ...
            "order": int(args.butter_order),
            "cutoff_hz": float(args.butter_cutoff_hz),
            "fps": float(args.butter_fps),
            "zero_lookahead": True,
            "initialized_from_first_valid_sample": True,
            "filtered_segment": [int(valid.nonzero()[0]), int(valid.nonzero()[-1]) + 1],
        },
        "gt_smoother": {
            "type": "centered_moving_average",
            "window": GT_MA_WINDOW,
            "realtime": False,
            "target_only": True,
        },
    ...
    return {
        ...
        "aIMU_leaf_rel_butter2_4hz": aIMU_leaf_rel_butter.cpu(),
        "aGT_leaf_rel_centered_ma9": aGT_leaf_rel_centered.cpu(),
        "aGT_leaf_rel_butter2_4hz": aGT_leaf_rel_butter.cpu(),
        "aM_raw": aM_raw.cpu(),
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build asymmetric leaf-relative acceleration residual audit cache v5.")
    ...
    parser.add_argument("--butter-cutoff-hz", type=float, default=BUTTER_CUTOFF_HZ)
    parser.add_argument("--butter-fps", type=float, default=FPS)
...
def main() -> None:
    args = parse_args()
    if int(args.butter_order) != BUTTER_ORDER or abs(float(args.butter_cutoff_hz) - BUTTER_CUTOFF_HZ) > 1e-9:
        raise ValueError("This v5 experiment contract requires --butter-order 2 and --butter-cutoff-hz 4.0")
    build_all(args)
