#!/usr/bin/env python3
"""Build leaf-relative causal Butterworth acceleration residual audit caches.
...
The realtime smoother is a zero-lookahead causal Butterworth low-pass filter.
It is applied only to the centered-difference valid segment so NaN boundary
frames from GT finite differences never enter the IIR filter state.
"""
...
DEFAULT_OUTPUT_ROOT = Path("data/experiments/acc_leaf_relative_residual_v4_causal_butterworth_20260618")
ROOT_INDEX = 5
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build leaf-relative causal Butterworth acceleration residual audit cache v4.")
...
def main() -> None:
    ...
    if int(args.butter_order) != BUTTER_ORDER or abs(float(args.butter_cutoff_hz) - BUTTER_CUTOFF_HZ) > 1e-9:
        raise ValueError("This v4 experiment contract requires --butter-order 2 and --butter-cutoff-hz 4.0")
    build_all(args)
