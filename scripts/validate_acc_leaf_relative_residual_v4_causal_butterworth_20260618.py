#!/usr/bin/env python3
"""Validate leaf-only causal Butterworth acceleration residual audit caches."""
...
DEFAULT_ROOT = Path("data/experiments/acc_leaf_relative_residual_v4_causal_butterworth_20260618")
...
COMPARISONS = {
    ...
    "butter2_4hz_leaf_relative": ("aIMU_leaf_rel_butter2_4hz", "aGT_leaf_rel_butter2_4hz"),
}
...
def validate_record(record: dict, path: str) -> None:
    ...
    for key in ("aIMU_leaf_rel_raw", "aGT_leaf_rel_raw", "aIMU_leaf_rel_butter2_4hz", "aGT_leaf_rel_butter2_4hz"):
...
def row_for(comparison: str, record: dict, path: str) -> tuple[list[dict], dict]:
    ...
    debug = {
        ...
        "zero_lookahead": bool(meta.get("realtime_smoother", {}).get("zero_lookahead", False)),
        "initialized_from_first_valid_sample": bool(meta.get("realtime_smoother", {}).get("initialized_from_first_valid_sample", False)),
    }
    return rows, debug
...
def sequence_metrics(record: dict, path: str) -> tuple[list[dict], dict]:
    ...
    rows = []
    debug = None
    for comparison in ("raw_leaf_relative", "butter2_4hz_leaf_relative", "centered_ma9_oracle"):
...
def sensor_table(rows: List[dict]) -> str:
    ...
    for sensor in SUMMARY_SENSOR_ORDER:
        ...
        butter = find_row(rows, "butter2_4hz_leaf_relative", "ALL", sensor)
        lines.append(
...
def primary_table(rows: List[dict]) -> str:
    ...
    for dataset in ("DIP", "TotalCapture"):
        for comparison in ("raw_leaf_relative", "butter2_4hz_leaf_relative"):
            try:
...
def decision(aggregate: List[dict], v3_ref: dict) -> tuple[str, dict]:
    try:
        ...
        dip_butter = find_row(aggregate, "butter2_4hz_leaf_relative", "DIP")
        tc_raw = find_row(aggregate, "raw_leaf_relative", "TotalCapture")
        tc_butter = find_row(aggregate, "butter2_4hz_leaf_relative", "TotalCapture")
    except KeyError:
    ...
    dip_rmse_ratio = dip_butter["rmse"] / dip_ref.get("rmse", dip_butter["rmse"]) if dip_ref else float("nan")
    tc_rmse_ratio = tc_butter["rmse"] / tc_ref.get("rmse", tc_butter["rmse"]) if tc_ref else float("nan")
    improves_dip = dip_butter["l2"] < dip_raw["l2"] and dip_butter["rmse"] < dip_raw["rmse"] and dip_butter["corr"] > dip_raw["corr"]
    improves_tc = tc_butter["l2"] < tc_raw["l2"] and tc_butter["rmse"] < tc_raw["rmse"] and tc_butter["corr"] > tc_raw["corr"]
    pass_gate = improves_dip and improves_tc and dip_butter["corr"] >= 0.94 and tc_butter["corr"] >= 0.94 and dip_rmse_ratio <= 1.2 and tc_rmse_ratio <= 1.2
    soft_pass_gate = improves_dip and improves_tc and dip_butter["corr"] >= 0.93 and tc_butter["corr"] >= 0.93 and dip_rmse_ratio <= 1.35 and tc_rmse_ratio <= 1.35
    if pass_gate:
        label = "pass"
    elif soft_pass_gate:
        label = "soft-pass"
    ...
    return label, {
        ...
        "dip_corr": dip_butter["corr"],
        "totalcapture_corr": tc_butter["corr"],
    }
...
def build_summary(result: dict) -> str:
    ...
    lines = [
        ...
        "",
        f"- v3 DIP centered_ma9 L2/RMSE/corr = {fmt(dip_ref.get('l2', float('nan')))} / {fmt(dip_ref.get('rmse', float('nan')))} / {fmt(dip_ref.get('corr', float('nan')))}",
        f"- v3 TotalCapture centered_ma9 L2/RMSE/corr = {fmt(tc_ref.get('l2', float('nan')))} / {fmt(tc_ref.get('rmse', float('nan')))} / {fmt(tc_ref.get('corr', float('nan')))}",
        f"- butter2_4hz improves over raw on DIP and TC: `{decision_debug['improves_dip'] and decision_debug['improves_totalcapture']}`",
        f"- DIP butter/oracle RMSE ratio: `{fmt(decision_debug['dip_rmse_ratio_vs_v3_oracle'])}`",
        f"- TotalCapture butter/oracle RMSE ratio: `{fmt(decision_debug['totalcapture_rmse_ratio_vs_v3_oracle'])}`",
        "",
        "## Decision Rule",
        "",
        f"Decision: `{label}`.",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Validate leaf-relative causal Butterworth acceleration residual audit v4.")
    ...
    checks = {
        "shape_consistency": all(
            rec["aIMU_leaf_rel_raw"].shape == rec["aGT_leaf_rel_raw"].shape == rec["aIMU_leaf_rel_butter2_4hz"].shape == rec["aGT_leaf_rel_butter2_4hz"].shape
