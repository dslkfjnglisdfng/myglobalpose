...
ROOT=${ROOT:-data/experiments/newpose_ctrl_v2_fk_leaf_20260608}
export ROOT
...
STAGE_A_BATCH=${STAGE_A_BATCH:-32}
STAGE_B_BATCH=${STAGE_B_BATCH:-16}
export STAGE_A_BATCH STAGE_B_BATCH
...
train_newpose() {
  ...
  if [[ ! -f "$out_dir/train_result.json" ]]; then
    local args=(
      ...
      --model-type newpose_ctrl_v2_fk_leaf
      --loss-preset v2_fk_leaf
      --selection-metric fk_leaf_physical
...
# Smoke: cache, train, and module eval. Full-pipeline 11 metrics are disabled
# by default for this module-design iteration.
...
if [[ ! -f "$EVAL_ROOT/smoke_module.json" ]]; then
  "$PY" newpose_ctrl_eval.py \
    ...
    --version-name newpose_ctrl_v2_fk_leaf_smoke \
    --module-only \
...
train_newpose \
  ...
  newpose_ctrl_v2_fk_leaf_stage_a_amass_pretrain \
  50 1e-4 1e-6 "$STAGE_A_BATCH" "" 50
...
train_newpose \
  ...
  newpose_ctrl_v2_fk_leaf_stage_b_dip_finetune \
  40 2e-5 2e-7 "$STAGE_B_BATCH" \
...
if [[ "$RUN_MODULE_EVAL" == "1" ]]; then
  ...
  eval_newpose_module "$ROOT/stage_a_amass_pretrain/best_loss.pt" "$CACHE_ROOT/dip_test_newpl_amass/newpose_ctrl_cache_manifest.json" "$EVAL_MODULE_ROOT/dip_stage_a_best_module.json" DIP-IMU-test newpose_ctrl_v2_fk_leaf_stage_a_best
  eval_newpose_module "$ROOT/stage_a_amass_pretrain/best_loss.pt" "$CACHE_ROOT/tc_test_newpl_amass/newpose_ctrl_cache_manifest.json" "$EVAL_MODULE_ROOT/tc_stage_a_best_module.json" TotalCapture-test newpose_ctrl_v2_fk_leaf_stage_a_best
  eval_newpose_module "$ROOT/stage_b_dip_finetune/best_loss.pt" "$CACHE_ROOT/dip_test_newpl_dip/newpose_ctrl_cache_manifest.json" "$EVAL_MODULE_ROOT/dip_stage_b_best_module.json" DIP-IMU-test newpose_ctrl_v2_fk_leaf_stage_b_best
  eval_newpose_module "$ROOT/stage_b_dip_finetune/best_loss.pt" "$CACHE_ROOT/tc_test_newpl_dip/newpose_ctrl_cache_manifest.json" "$EVAL_MODULE_ROOT/tc_stage_b_best_module.json" TotalCapture-test newpose_ctrl_v2_fk_leaf_stage_b_best
  eval_newpose_module "$ROOT/stage_b_dip_finetune/last.pt" "$CACHE_ROOT/dip_test_newpl_dip/newpose_ctrl_cache_manifest.json" "$EVAL_MODULE_ROOT/dip_stage_b_last_module.json" DIP-IMU-test newpose_ctrl_v2_fk_leaf_stage_b_last
  eval_newpose_module "$ROOT/stage_b_dip_finetune/last.pt" "$CACHE_ROOT/tc_test_newpl_dip/newpose_ctrl_cache_manifest.json" "$EVAL_MODULE_ROOT/tc_stage_b_last_module.json" TotalCapture-test newpose_ctrl_v2_fk_leaf_stage_b_last
fi
...
if [[ "$RUN_FULL_EVAL" == "1" ]]; then
  ...
  eval_newpose "$ROOT/stage_a_amass_pretrain/best_loss.pt" "$CACHE_ROOT/dip_test_newpl_amass/newpose_ctrl_cache_manifest.json" "$EVAL_ROOT/dip_stage_a_best.json" DIP-IMU-test newpose_ctrl_v2_fk_leaf_stage_a_best "$FULL_EVAL_MAX_SEQUENCES"
  eval_newpose "$ROOT/stage_a_amass_pretrain/best_loss.pt" "$CACHE_ROOT/tc_test_newpl_amass/newpose_ctrl_cache_manifest.json" "$EVAL_ROOT/tc_stage_a_best.json" TotalCapture-test newpose_ctrl_v2_fk_leaf_stage_a_best "$FULL_EVAL_MAX_SEQUENCES"
  eval_newpose "$ROOT/stage_b_dip_finetune/best_loss.pt" "$CACHE_ROOT/dip_test_newpl_dip/newpose_ctrl_cache_manifest.json" "$EVAL_ROOT/dip_stage_b_best.json" DIP-IMU-test newpose_ctrl_v2_fk_leaf_stage_b_best "$FULL_EVAL_MAX_SEQUENCES"
  eval_newpose "$ROOT/stage_b_dip_finetune/best_loss.pt" "$CACHE_ROOT/tc_test_newpl_dip/newpose_ctrl_cache_manifest.json" "$EVAL_ROOT/tc_stage_b_best.json" TotalCapture-test newpose_ctrl_v2_fk_leaf_stage_b_best "$FULL_EVAL_MAX_SEQUENCES"
  eval_newpose "$ROOT/stage_b_dip_finetune/last.pt" "$CACHE_ROOT/dip_test_newpl_dip/newpose_ctrl_cache_manifest.json" "$EVAL_ROOT/dip_stage_b_last.json" DIP-IMU-test newpose_ctrl_v2_fk_leaf_stage_b_last "$FULL_EVAL_MAX_SEQUENCES"
  eval_newpose "$ROOT/stage_b_dip_finetune/last.pt" "$CACHE_ROOT/tc_test_newpl_dip/newpose_ctrl_cache_manifest.json" "$EVAL_ROOT/tc_stage_b_last.json" TotalCapture-test newpose_ctrl_v2_fk_leaf_stage_b_last "$FULL_EVAL_MAX_SEQUENCES"
fi
...
root = Path(os.environ.get("ROOT", "data/experiments/newpose_ctrl_v2_fk_leaf_20260608"))
eval_root = root / "eval"
evals = {}
...
summary = {
    "status": "ok",
    "root": str(root),
    "protocol": "newpose_ctrl_v2_fk_leaf: NewPL stream -> pose control points with FK leaf/joint physical losses; AMASS pretrain -> DIP fine-tune -> DIP/TC module eval.",
    ...
    "loss": "v2_fk_leaf preset: v1 control/state losses plus FK leaf position/velocity/acceleration and FK joint position in root/body coordinates.",
    "selection_metric": "fk_leaf_physical: FK_leaf_L2_cm + 0.25*FK_leaf_vel + 0.10*FK_leaf_acc + 0.10*FK_joint_L2_cm + 0.05*gR_pose.",
    "batch_defaults": {
        "stage_a_amass": int(os.environ.get("STAGE_A_BATCH", "32")),
        "stage_b_dip": int(os.environ.get("STAGE_B_BATCH", "16")),
    },
    "checkpoints": {
