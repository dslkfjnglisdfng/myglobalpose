#!/usr/bin/env python3
"""Summarize NewIK1 v9 adaptive loss-search trial outputs into a ranking table."""
...
ROOT = Path("data/experiments/newik1_v9_adaptive_loss_search")
OUT = ROOT / "summary" / "phase1_ranking.json"

BASELINES = {
    "newpl_init36_s4": 38.625657482802865,
    "v8_best_s4": 38.69415222530066,
...
def summarize_checkpoint(trial_dir: Path, ckpt: str) -> Dict[str, Any]:
    ...
    if s4 and s4.get("status") == "ok":
        row.update({
            ...
            "s4_delta_vs_v8": float(s4["score"]) - BASELINES["v8_best_s4"],
    ...
    if gt and gt.get("status") == "ok":
        ...
        row.update({
            "pRJ_cm_l2": float(new.get("pRJ_cm_l2", new["pRJ_l2"] * 100.0)),
            "pRJ_l2": float(new["pRJ_l2"]),
            "leaf_pRJ_cm_l2": float(new["leaf_pRJ_cm_l2"]) if "leaf_pRJ_cm_l2" in new else None,
            "leaf_pRJ_l2": float(new["leaf_pRJ_l2"]) if "leaf_pRJ_l2" in new else None,
            "gR2_angle_deg": float(new["gR2_angle_deg"]),
            "pRJ_dot_l2": float(new["pRJ_dot_l2"]) if "pRJ_dot_l2" in new else None,
            "pRJ_ddot_l2": float(new["pRJ_ddot_l2"]) if "pRJ_ddot_l2" in new else None,
            "leaf_pRJ_dot_l2": float(new["leaf_pRJ_dot_l2"]) if "leaf_pRJ_dot_l2" in new else None,
            "leaf_pRJ_ddot_l2": float(new["leaf_pRJ_ddot_l2"]) if "leaf_pRJ_ddot_l2" in new else None,
            "gR2_dot_l2": float(new["gR2_dot_l2"]) if "gR2_dot_l2" in new else None,
            "gR2_ddot_l2": float(new["gR2_ddot_l2"]) if "gR2_ddot_l2" in new else None,
            "state_l2": float(new["state_l2"]),
            "pRJ_cm_l2_delta": float(delta.get("pRJ_cm_l2", delta["pRJ_l2"] * 100.0)),
            "pRJ_l2_delta": float(delta["pRJ_l2"]),
            "leaf_pRJ_cm_l2_delta": float(delta["leaf_pRJ_cm_l2"]) if "leaf_pRJ_cm_l2" in delta else None,
            "leaf_pRJ_l2_delta": float(delta["leaf_pRJ_l2"]) if "leaf_pRJ_l2" in delta else None,
            "gR2_angle_delta": float(delta["gR2_angle_deg"]),
            "pRJ_dot_l2_delta": float(delta["pRJ_dot_l2"]) if "pRJ_dot_l2" in delta else None,
            "pRJ_ddot_l2_delta": float(delta["pRJ_ddot_l2"]) if "pRJ_ddot_l2" in delta else None,
            "leaf_pRJ_dot_l2_delta": float(delta["leaf_pRJ_dot_l2"]) if "leaf_pRJ_dot_l2" in delta else None,
            "leaf_pRJ_ddot_l2_delta": float(delta["leaf_pRJ_ddot_l2"]) if "leaf_pRJ_ddot_l2" in delta else None,
            "gR2_dot_l2_delta": float(delta["gR2_dot_l2"]) if "gR2_dot_l2" in delta else None,
            "gR2_ddot_l2_delta": float(delta["gR2_ddot_l2"]) if "gR2_ddot_l2" in delta else None,
...
def main() -> None:
    ...
    for trial_dir in sorted(p for p in args.root.iterdir() if p.is_dir() and p.name.startswith("v9_")):
