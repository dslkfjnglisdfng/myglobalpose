#!/usr/bin/env python3
"""Build root-IMU-relative acceleration caches for acc invariance v2.

Contract:
  IMU input: aM_rel = smooth(aM) - smooth(aM)[:, root_imu]
  GT target: aGT_rel = diff_acc(FK_zero_translation(p_WJ + R_WJ @ r_JS))
                    - diff_acc(FK_zero_translation(root_imu_site))

The GT path uses SMPL forward kinematics with tran forced to zero for every
dataset.  Both IMU and GT are model/world-frame vectors, made invariant to root
translation by subtracting the pelvis/root IMU acceleration at index 5.
"""
...
DEFAULT_OUTPUT_ROOT = Path("data/experiments/acc_invariance_datacache_v2_20260618")
ROOT_INDEX = 5
...
def load_smoothed_imu(data: dict, seq_idx: int, args: argparse.Namespace) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, str]:
    aM_raw, wM, RMB = official_imu_fields(data, seq_idx)
    if data.get("aM_smooth") and data["aM_smooth"]:
        aM_smooth = data["aM_smooth"][seq_idx].float()
        source = "aM_smooth"
    else:
        aM_smooth = smooth_centered(aM_raw, args.smooth_window, mode=args.smoothing_mode)
        source = f"smooth_centered(aM, window={args.smooth_window}, mode={args.smoothing_mode})"
    return aM_raw.float(), aM_smooth.float(), wM.float(), RMB.float(), source
...
def build_record(
    ...
    aM_raw, aM_smooth, wM, RMB, imu_source = load_smoothed_imu(data, seq_idx, args)
    n = min(pose.shape[0], aM_raw.shape[0], aM_smooth.shape[0], wM.shape[0], RMB.shape[0])
    if args.max_frames:
    ...
    pose = pose[:n]
    aM_raw = aM_raw[:n]
    aM_smooth = aM_smooth[:n]
    wM = wM[:n]
    RMB = RMB[:n]
    p_ws = sensor_sites_zero_translation(pose, offset_r, body_model, device, args.fk_batch_size)
    aGT_raw, diff_valid = centered_second_difference(p_ws, DT)
    aM_rel = aM_smooth - aM_smooth[:, ROOT_INDEX:ROOT_INDEX + 1]
    aGT_rel = aGT_raw - aGT_raw[:, ROOT_INDEX:ROOT_INDEX + 1]
    valid = diff_valid & torch.isfinite(aM_rel).all(dim=(-1, -2)) & torch.isfinite(aGT_rel).all(dim=(-1, -2))
    ...
    aGT_raw = torch.nan_to_num(aGT_raw, nan=0.0)
    aGT_rel = torch.nan_to_num(aGT_rel, nan=0.0)
    meta = {
        ...
        "formulation": "rootIMU_relative_acceleration",
        "zero_translation": True,
        "pose_source": pose_source,
        "imuacc_source": imu_source,
        "offset_source": offset_source,
        "sensor_names": list(SENSOR_NAMES),
        "sensor_joint_mapping": list(IMU_JOINTS),
        "offset_contract": COORDINATE_CONTRACT,
        "frame_contract": "M/world-frame acceleration vectors with root-IMU acceleration subtracted; no sensor-local rotation is applied.",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build acc invariance datacache v2.")
