...
ACC_CURVE_V3_INPUT_SIZE = 99
ACC_CURVE_V3_STATE_DIM = 15
...
def acc_curve_v3_leafrel_features(a_imu_leaf_rel_raw, a_imu_leaf_rel_butter2_4hz, wM, RMB):
    """Build v1-style model/world-frame AccCurve features for leaf-relative acceleration.

    Layout:
      raw leaf-relative acc[15]
      + causal Butterworth leaf-relative acc[15]
      + raw-minus-butter residual[15]
      + wM[18]
      + RMB_6d[36]
      = 99D.
    """
    ...
    return torch.cat(
        (
            a_imu_leaf_rel_raw.reshape(a_imu_leaf_rel_raw.shape[0], 15),
            a_imu_leaf_rel_butter2_4hz.reshape(a_imu_leaf_rel_butter2_4hz.shape[0], 15),
            (a_imu_leaf_rel_raw - a_imu_leaf_rel_butter2_4hz).reshape(
                a_imu_leaf_rel_raw.shape[0], 15
            ),
...
class PLStyleAccCurveV3LeafRelModule(torch.nn.Module):
    """PLCurve-style residual curve over 15D leaf-relative acceleration."""
    ...
    def __init__(
        ...
        super().__init__()
        if input_size != ACC_CURVE_V3_INPUT_SIZE:
            raise ValueError(f"AccCurve v3 expects 99D input, got {input_size}.")
        if state_dim != ACC_CURVE_V3_STATE_DIM:
            raise ValueError(f"AccCurve v3 predicts 15D leaf acceleration, got {state_dim}.")
        if tail_update != 4:
            raise ValueError("AccCurve v3 keeps the PL-style L=4 tail-update contract.")
    ...
    def step(self, feature_t, base_t):
        ...
        if self.control_buffer is None:
            ...
            self.control_buffer = torch.cat(
                (old_control, tail_control, new_control.unsqueeze(1)), dim=1
            )
    ...
    def forward_sequence(self, features, base):
        ...
        for i in range(features.shape[0]):
            ...
            preds.append(out["pred_leaf_rel_acc_t"])
        ...
        result = {
            "pred_leaf_rel_acc": torch.stack(preds),
        ...
        if squeeze_batch:
            for key in ("pred_leaf_rel_acc", "base", "residual", "new_control"):
