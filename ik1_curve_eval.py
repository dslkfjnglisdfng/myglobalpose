...
from ik1_curve import IK1CurveModule
...
from pl_curve_eval import build_pl_curve
from test import MotionEvaluator


def build_ik1_curve(checkpoint_path):
...
@torch.no_grad()
def run_sequence(record, pl_curve=None, ik1_curve=None):
    net = GPNet(
        pl_backend='curve_v1' if pl_curve is not None else 'original',
        pl_curve_module=pl_curve,
        ik1_backend='curve_v1' if ik1_curve is not None else 'original',
    ...
    for i in range(record['pose_gt'].shape[0]):
        pose[i], tran[i] = net.forward_frame(
            record['aM'][i].to(DEVICE),
            record['wM'][i].to(DEVICE),
            record['RMB'][i].to(DEVICE),
        )
...
def main():
    parser = argparse.ArgumentParser(description='Evaluate IK1Curve_v1 inside GPNet.')
    ...
    result = {
        ...
        'pl_backend': 'curve_v1',
        'ik1_backend': 'curve_v1' if args.ik1_checkpoint else 'original',
    ...
    try:
        ...
        if args.ik1_checkpoint:
            ik1_curve, ik1_config = build_ik1_curve(args.ik1_checkpoint)
        rows = evaluate(records, pl_curve=pl_curve, ik1_curve=ik1_curve, max_eval_sequences=args.max_eval_sequences)
