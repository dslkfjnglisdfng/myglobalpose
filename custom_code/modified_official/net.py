...
class GPNet(torch.nn.Module):
    ...
    def __init__(
        ...
        if self.pl_backend not in ('original', 'curve_v1', 'va_state_v1'):
        ...
        if self.pl_backend == 'va_state_v1' and self.pl_va is None:
            raise ValueError('pl_backend=va_state_v1 requires pl_va_module.')
    ...
    def _run_pl_stage(self, x_pl_in, x_curve_in=None):
        if self.pl_backend == 'va_state_v1':
            result = self.pl_va.step(x_curve_in)
            pl_out = result['pl_t'][0]
            self.last_pl_curve_debug = dict(self.pl_va.last_debug)
            return pl_out, result['gR1'][0]
    ...
    def forward_frame(
        ...
        # PL-s1
        x = torch.cat((aRB.ravel(), wRB.ravel(), RRB.ravel(), gR0))
        x_curve = self._pl_curve_frame_feature(a, w_pl, R, x)
        ...
        self.last_w_source_debug = {
            'pRB': pl_output[:15].detach().cpu(),
            'gR1': gR1.detach().cpu(),
            'pRJ': ik1_output[:69].detach().cpu(),
            'gR2': gR2.detach().cpu(),
            'w_pl_M': w_pl.detach().cpu(),
            'w_vr_M': w_vr.detach().cpu(),
            'wRB_pl': w_pl.mm(R[5]).detach().cpu(),
            'wRB_vr': w_vr.cpu().mm(pose[0]).detach().cpu(),
        }
