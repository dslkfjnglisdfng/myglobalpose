...
| `PL-s1 / joint-leaf acceleration` | `newpl_joint_leaf_acc_20260619`, `pl_joint_control_acc_aug102_v1` | `pl_joint_control_acc_aug102_v1` full module run completed; DIP finetune gives negligible joint-position change and no derivative/gravity improvement; no full-pipeline claim |
...
| `AccCurve / acceleration residual` | `acc_curve_v1_20260617`, `acc_curve_v1_totalcapture_eval_20260618`, `acc_curve_v1_totalcapture_zero_trans_eval_20260618`, `acc_curve_v1_fulltrans_rootacc_reconstruction_eval_20260618`, `acc_curve_v2_gtfk_q_qdot_qddot_rjs_20260617`, `acc_curve_pl_input_eval_20260617`, `acc_invariance_datacache_v2_rebuild_20260618`, `acc_leaf_relative_residual_v3_20260618`, `acc_leaf_relative_residual_v4_causal_butterworth_20260618`, `acc_leaf_relative_residual_v5_imu_causal_gt_centered_20260618`, `acc_curve_v3_leafrel_causal_butter_20260618`, `acc_curve_v3_error_distribution_rjs_audit_20260618` | standalone acceleration-level modules and audits; v1 is now interpreted as root-translation-free sensor-site acceleration, and the old TC full-trans failure was target-mismatched but not fully fixed by adding GT root acceleration back. Current v3 failure diagnostic finds likely model overfit / TC overcorrection; do not claim downstream pose improvement |
...
TotalCapture acceleration-fit `r_JS` diagnostic update on 2026-07-04: `totalcapture_accfit_rjs_20260704` solves `r_JS` directly from the sensor-frame specific-force objective
`a_S = R_WS^T @ (ddot(p_WJ) + ddot(R_WJ) @ r_JS - g_W)` on `data/dataset_work/TotalCapture_globalpose_official/test.pt`.
It does not train or change PL/IK/VR. Main SavGol-9 sensor-specific-force RMSE:
vertex baseline `2.742093`, old footlock rJS `3.856079`, accfit per-sequence `2.636344`, accfit global `2.640858`, accfit LOO `2.642856`.
Lower legs and forearms improve versus old rJS; projected rJS norms stay below `0.249660 m`, condition numbers are healthy (median `1.390331`, max `1.848262`), and with-bias adds only `0.036158` mean fit-improvement.
Conclusion: old footlock/pseudo rJS was not suitable as an acceleration target, but acceleration-fit rJS is promising for TotalCapture acceleration explainability. Before using it for supervision, choose per-sequence/global/LOO policy and audit residual/time-alignment behavior.

TotalCapture both-smooth FK/IMU audit update on 2026-07-05: `totalcapture_both_smooth_fk_imu_acc_20260705` extends the old smooth protocol on the TotalCapture official test split using raw, legacy MA9, legacy lowpass-5Hz, SavGol-9/15, centered MA9/15/21, and lowpass 3/5/8/12Hz in both sensor-frame specific-force and model/world-frame comparisons. With accfit global rJS, sensor-frame raw RMSE is `3.053103`, while centered MA21 RMSE is `0.492988`, L2 `0.612267`, corr `0.996950`, cosine `0.996965`, p95 `1.759593`. Lower-leg-only accfit global rJS raw RMSE is `3.613702`, while centered MA21 is `0.430999`. SavGol-9 accfit global beats old rJS by `-1.939099` RMSE and vertex by `-0.204214`; lowpass-5Hz beats old rJS by `-1.797249` and vertex by `-0.205957`. Conclusion: old smooth protocols help because both noisy IMU and noisy FK second differences are low-passed; improvement is not merely a frame artifact because both frames are reported and centered MA21 wins in both. Future pose smoothing should chase smooth/low-frequency acceleration only, not raw acceleration.

Bounded smooth refinement update on 2026-07-05: `refine_totalcapture_pose_kalman_style_smoother.py` and `alternate_totalcapture_rjs_pose_smoothing.py` are implemented as data diagnostics. The 4-sequence, 180-frame, 20-iteration centered-MA21 smoother did not pass: all-sensor RMSE delta `+0.000350`, held-out lower-leg delta `+0.000370`, with tiny gyro/jerk/foot-sliding regressions. The 3-round alternating bounded run also did not pass: round-3 all-sensor delta `+0.000150`, held-out lower-leg delta `+0.000119`. These are not selected refinement recipes.

q-smoothing FK/IMU diagnostic update on 2026-07-07: `q_smoothing_fk_imu_acc_consistency_20260707` evaluates only processed `q(t)=[tran, pose]`, derives `qd/qdd` from the same processed trajectory, recomputes `p_WS=p_WJ+R_WJ@r_JS`, and uses strict centered second differences for FK acceleration. It explicitly does not smooth `a_pred_world` or `a_pred_sensor`; only the IMU target uses `centered_ma21(aS)` and `lowpass_5hz(aS)`. Full TotalCapture official test result: centered-MA21 target best is `bspline_q_knot10` RMSE `2.751086`, L2 `2.844688`, corr `0.899847`, cosine `0.897525`, p95 `9.188947`; lowpass-5Hz target best is `raw_q` RMSE `2.598585`. No B-spline method supports control-point refinement because the B-spline q deviations are far beyond the acceptance gate; knot10 has mean translation delta `0.775097 m` and mean pose delta `17.990561 deg`. Decision: diagnostic only; do not enter B-spline control-point refinement from this version.
...
| newpl_joint_leaf_acc_20260619 | PL-s1 module route only | switch NewPL cache/eval target from vertex PL to SMPL joint-leaf positions, with optional frozen leaf-acc predictor features | three modes: `baseline_jointtarget_84D=aRB+wRB+RRB+gR0`, `acc_root_102D=84D+a_smoothed_R+a_output_R`, `acc_mixed_102D=84D+a_smoothed_W+a_output_W` | 18D target is `p_leaf_joint_R[15]+gR1[3]`; corrected `pl_base` is official GPNet PL-s1 pre-correction prediction from legacy 84D IMU input, not pose_prephysics FK; eval logs `protocol_check` | no new loss; original `pl_curve_loss`, derivative-aware target controls, init36, spline decoder, and `control_physical` selection | original full run was completed but used wrong pose_prephysics FK base; DIP test caches have now been regenerated/validated with official PL-s1 base only | old result invalid for v5 comparability; repaired DIP test cache has `base_gR1=15.267228 deg` for all three modes, matching old v5 official DIP test cache exactly | not applicable; no IK/full-pipeline by contract | protocol repaired; must retrain from corrected caches before making effectiveness claims |
| pl_joint_control_acc_aug102_v1 | PL-s1 module route only | new joint-target control experiment with frozen joint acceleration auxiliary input; this is not a continuation of legacy vertex PL target | `frozen_joint_acc_aug102 = aRB[18]+wRB[18]+RRB[45]+gR0[3]+frozen_joint_acc_R[15]+root_acc_smooth_R[3]`; explicit feature_mode prevents confusion with older smooth_residual 102D layout | 18D target is `joint_pRB[15]+gR[3]`, where `joint_pRB=(SMPL joints [18,19,4,5,15]-root joint) @ root_R`; legacy vertex pRB is diagnostic only | joint-control loss weights: pRB 1.0, gR1 0.3, gt_control_pRB 0.5, gt_control_gR1 0.1, pRB_dot 0.5, pRB_ddot 0.1, pRB_ddot_smooth/gR_smooth/control_point_prior/tail_update_prior 0.001 | full AMASS 80 -> DIP 40 module run under `data/experiments/pl_joint_control_acc_aug102_v1_full_20260620_134805`; frozen `imu_leaf_acc_predictor_v1` loaded eval-only with checkpoint normalization | AMASS selected epoch 1 (`0.8822397489`); DIP selected epoch 37 (`0.4478730437`); DIP test after AMASS/DIP joint_pos 0.277489/0.277398 m, joint_vel 0.423087/0.423258 m/s, joint_acc 43.323986/43.328894 m/s^2, gravity 12.944853/12.946709 deg | no full S4/full-pipeline result; DIP finetune gives negligible joint-position improvement and no derivative/gravity improvement | diagnostic only for now; investigate derivative supervision/selection mismatch before any downstream evaluation |
...
| acc_invariance_datacache_v2_rebuild_20260618 | AccCurve / acceleration residual | root-IMU-relative acceleration cache rebuild for AMASS/DIP/TotalCapture | `aM_rel = aM_smooth - aM_smooth[:,5]`; GT target `aGT_rel = diff_acc(FK_zero_trans(p_WJ + R_WJ @ rJS)) - root` | `aM_rel[6,3]` and `aGT_rel[6,3]` plus raw/zero-trans debug tensors | root invariance, shape consistency, leakage corr, and overall/per-sensor L2/RMSE/corr | full AMASS + DIP + TotalCapture rebuild; central second difference with `dt=1/60`; cache-only rebuild | root invariance passes exactly; leakage test passes 1251/1404 sequences; mean corr(raw abs)=0.623005 vs mean corr(v2 relative)=0.617329 | n/a | diagnostic cache only; root leakage is reduced structurally, but v2 relative corr does not surpass raw absolute overall |
| acc_leaf_relative_residual_v3_20260618 | AccCurve / acceleration residual | leaf-only acceleration residual audit without root metric | raw comparison: `aM_leaf-aM_root` vs `diff_acc(p_leaf_zero_trans)-diff_acc(p_root_zero_trans)`; smooth comparison applies centered moving average window 9 directly to both leaf-relative tensors | `aIMU_leaf_rel_raw[5,3]`, `aGT_leaf_rel_raw[5,3]`, `aIMU_leaf_rel_smooth[5,3]`, `aGT_leaf_rel_smooth[5,3]`; root raw fields debug only | leaf-only L2/RMSE/MAE/corr/cosine/magnitude/axis metrics by dataset, split, and sensor | full AMASS + DIP + TotalCapture audit; 1404 sequences / 1609025 valid frames; no training and no PL/IK/VR/full-pipeline/S4 eval | yes for residual audit: ALL raw L2/RMSE/corr `1.372700/2.269143/0.857988`; ALL smooth `0.457061/0.562864/0.979057`; AMASS smooth L2/RMSE/corr `0.272780/0.389472/0.990993`; DIP `0.796990/0.791042/0.948895`; TC `1.011330/0.897591/0.945537` | not run | residual audit only; smoothing is necessary before IMU/FK acceleration residual comparison; root is reference-only and excluded from all residual metrics; no downstream pose-improvement claim |
| acc_leaf_relative_residual_v4_causal_butterworth_20260618 | AccCurve / acceleration residual | realtime leaf-only acceleration residual audit with causal Butterworth smoothing | raw comparison unchanged from v3; realtime comparison filters valid segment only with zero-lookahead causal Butterworth order=2 cutoff=4Hz, fps=60, initialized from first valid sample | `aIMU_leaf_rel_raw[5,3]`, `aGT_leaf_rel_raw[5,3]`, `aIMU_leaf_rel_butter2_4hz[5,3]`, `aGT_leaf_rel_butter2_4hz[5,3]`; root raw fields debug only | primary DIP/TotalCapture leaf-only L2/RMSE/MAE/corr/cosine/magnitude/axis metrics; AMASS synthetic sanity only | full AMASS + DIP + TotalCapture audit; 1404 sequences / 1609025 valid frames; no training and no PL/IK/VR/full-pipeline/S4 eval | yes/pass: DIP raw `2.360895/3.530190/0.610050` -> butter `0.893481/0.914904/0.943719`; TC raw `2.905783/4.418546/0.563076` -> butter `1.092481/1.050146/0.941474`; butter RMSE ratio vs v3 centered oracle DIP `1.156581`, TC `1.169961` | not run | residual audit only; causal Butterworth improves real-IMU explainability and stays within 20% of centered oracle, but this is not AccCurve/PL/NewPL training and makes no downstream pose claim |
| acc_leaf_relative_residual_v5_imu_causal_gt_centered_20260618 | AccCurve / acceleration residual | asymmetric leaf-only acceleration residual audit: realtime IMU smoothing against centered GT target smoothing | raw comparison unchanged from v3/v4; asymmetric comparison filters IMU valid segment with zero-lookahead causal Butterworth order=2 cutoff=4Hz and filters GT valid segment with centered moving average window=9 target-only | `aIMU_leaf_rel_raw[5,3]`, `aGT_leaf_rel_raw[5,3]`, `aIMU_leaf_rel_butter2_4hz[5,3]`, `aGT_leaf_rel_centered_ma9[5,3]`; optional `aGT_leaf_rel_butter2_4hz[5,3]`; root raw fields debug only | primary DIP/TotalCapture leaf-only L2/RMSE/MAE/corr/cosine/magnitude/axis metrics; v4 symmetric butter and v3 centered oracle references; AMASS synthetic sanity only | full AMASS + DIP + TotalCapture audit; 1404 sequences / 1609025 valid frames; no training and no PL/IK/VR/full-pipeline/S4 eval | no/fail: DIP raw `2.360895/3.530190/0.610050` -> asym `1.828599/2.060184/0.687890`; TC raw `2.905783/4.418546/0.563076` -> asym `2.480798/2.426839/0.656914`; asym/v4 RMSE ratios DIP `2.251804`, TC `2.310953`; asym/v3 RMSE ratios DIP `2.604393`, TC `2.703725` | not run | residual audit only; GT centered smoothing is target-only and not runtime available; asymmetric phase mismatch likely dominates, so retain v4 symmetric Butterworth as better realtime explainability target; no downstream pose-improvement claim |
| acc_curve_v3_leafrel_causal_butter_20260618 | AccCurve / acceleration residual | AccCurve v1-style residual acceleration module trained on leaf-relative causal Butterworth target | 99D feature from reusable project cache: `acc_raw[15]+acc_smooth[15]+acc_raw_minus_smooth[15]+wM[18]+RMB_6d[36]`; `RMB_6d=rotation[..., :, :2].transpose(-1,-2).reshape(...,6)`; feature z-score fit from AMASS train only | `pred_leaf_rel_acc[15] = [5,3]`; base `aIMU_leaf_rel_butter2_4hz[15]`; target `aGT_leaf_rel_butter2_4hz[15]`; root index 5 reference only and excluded from prediction/loss/metric | valid-mask MSE to target plus control prior; checkpoint selection by validation pred/base L2 ratio; leaf-only L2/RMSE/corr/cosine/mag metrics | project-level cache `data/dataset_work/AccCurveV3LeafRelCausalButter_20260618` from v4 audit cache; 1404 sequences / 1609025 valid frames; AMASS pretrain 30 epochs then DIP finetune 20 epochs; DIP/TC test eval only; no TC training for main claim | mixed/fail: AMASS best epoch 13 ratio `0.940240`; DIP finetune best epoch 16 DIP-val ratio `0.851720`; DIP test pred/base L2/RMSE/corr `0.990334/0.750235/0.958276` vs base `1.196030/0.924315/0.943321`, ratio `0.828017`; TotalCapture test pred `1.365116/1.065184/0.923813` vs base `1.052403/0.902071/0.946864`, ratio `1.297142` | not run | acceleration-level only; DIP improves strongly but TotalCapture worsens sharply, so do not promote; no PL/NewPL/full-pipeline/S4/downstream pose claim |
| acc_curve_v3_error_distribution_rjs_audit_20260618 | AccCurve / acceleration residual | diagnostic audit for AccCurve v3 DIP-to-TC failure | reads v4 leaf-relative causal Butterworth cache plus v3 checkpoint; computes base residual distribution, rJS distribution, rJS acceleration contribution, and learned correction transfer | leaf-only residual/correction metrics on `[T,5,3]`; root index 5 reference only and excluded from residual/correction metrics; pelvis rJS audited only as offset metadata | distribution distances, rJS Mahalanobis/outlier stats, offset/joint acceleration contribution ratios, correction cosine/corr, overcorrection and harmful rates | full v4 cache scan; A/B over AMASS/DIP/TC, C/D over real-IMU DIP/TC; no training, no PL/NewPL/full-pipeline/S4 eval | diagnostic result: DIP train vs TC test residual mean diff norm `0.809654`, FD `2.173161`, MMD `0.061301`, mean-vector cosine `0.268100`; rJS DIP train vs TC test mean diff norm `0.136650`, FD `0.034094`, cosine `0.961810`; rJS offset contribution similar DIP test `0.733277` vs TC test `0.756321`; TC correction harmful_rate `0.564954`, overcorrection `0.304232`, correction corr `0.227905` | not run | final diagnosis `likely_model_overfit`; TC failure is more consistent with DIP-specific learned correction / TC overcorrection than rJS target mismatch; next: use base only for TC-like distribution, then test smaller residual_scale or sensor-specific residual gate |
| acc_curve_v1_20260617 | AccCurve / acceleration residual | PL-style curve residual over root-translation-free sensor-site acceleration in the v1 historical target namespace | 108D: `aM_raw[18]+aM_smooth[18]+residual[18]+wM[18]+RMB_6d[36]` | `pred_aM_curve[18]` root-translation-free acceleration; base `aM_smooth[18]` | valid-mask MSE to `aFK_smooth[18]`; selection by `val_pred_base_ratio` | AMASS 30 -> DIP 20; window 240, stride 120, batch 64; module eval only | mixed; DIP test ratio `0.6220`; direct TC full-trans pred_zero-vs-GT_full ratio `2.3940` was target-mismatched; corrected full-trans reconstruction ratio `1.619925` still fails baseline | not run | diagnostic only; v1 is not a full absolute acceleration predictor and should not be used for cross-dataset NewPL retrain without a stronger gate |
| acc_curve_v1_totalcapture_eval_20260618 | AccCurve / acceleration residual | historical wrong-match check: evaluates v1 zero/root-translation-free prediction directly against TotalCapture full-trans target | same v1 108D input from TC official offset cache | `pred_aM_curve[18]` vs full-trans `aFK_smooth[18]` | no training; target is `smooth(diff_acc(trans + p_WJ + R_WJ @ rJS))`, not strict GTFK | TC test cache build + eval only; checkpoint `acc_curve_v1_20260617/dip_finetune/best_loss.pt` | no; wrong pred_zero-vs-GT_full L2/RMSE `2.091960/1.539445` vs base `0.873843/0.693060`, ratio `2.393977`, corr `0.866428` | not run | keep only as unfair target-mismatch reference; do not call this alone a TC generalization failure |
| acc_curve_v1_totalcapture_zero_trans_eval_20260618 | AccCurve / acceleration residual | evaluate v1 checkpoint on TotalCapture zero-translation diff-pos target | same v1 108D input from TC official offset cache; target translation forcibly zeroed | `pred_aM_curve[18]` vs `aFK_smooth[18]` | no training; target is `smooth(diff_acc(p_WS_zero_trans))`, not strict GTFK | TC zero-trans cache build + eval only; checkpoint `acc_curve_v1_20260617/dip_finetune/best_loss.pt` | yes; TC zero-trans pred/base ratio `0.772415`, pred L2/RMSE `1.415560/0.977232` vs base `1.832642/1.466451`, corr `0.945382` | not run | confirms v1 predicts root-translation-free acceleration; corrected full-trans reconstruction still fails baseline, so zero-trans success is not a full-trans success claim |
| acc_curve_v1_fulltrans_rootacc_reconstruction_eval_20260618 | AccCurve / acceleration residual | corrected v1 full-trans evaluation by adding root translational acceleration back to the zero-trans prediction | same v1 108D input; zero-trans cache for prediction, full-trans cache for GT, source `tran_gt` for root-acc sanity | `pred_zero + a_root_trans_smooth[None]` vs full-trans `aFK_smooth[18]`; baseline is v1 decoded `aM_smooth` | no training; acceleration-level only; no PL/NewPL/full-pipeline/S4 claim | TC test eval only; verifies `GT_full ~= GT_zero + root_trans_acc`, max/mean/rmse `0.000366449/0.000013115/0.000026308` | no; corrected all-6 L2/RMSE/corr `1.415560/0.977232/0.949590` vs baseline `0.873843/0.693060/0.974734`, ratio `1.619925`; optional IMU-root estimate ratio `1.378504` | not run | old full-trans failure was target-mismatched, but root reconstruction still does not beat full-trans baseline; target mismatch explains part, not all, of the failure |
...
Original contract: `RNNWithInit(input_linear=False,input_size=84,output_size=18,hidden_size=512,num_rnn_layer=3,dropout=0.4)`; input `aRB[18]+wRB[18]+RRB[45]+gR0[3]`; output `pRB[15]+gR1[3]`; init `pRL[15]+gR0[3]`; IK1 input `RRB_after_PL[45]+gR1[3]+pRB[15]=63D`.

New contract: causal 102D input `aRB_raw[18]+wRB_rmbdiff[18]+RRB[45]+gR0[3]+aRB_smooth[18]`; raw RNN output `vRB_direct[15]+aRB_leaf[15]+gR1_raw[3]=33D`; explicit beta=0.7 trapezoidal state update returns legacy `pRB_state[15]+normalize(gR1_raw)[3]=18D`.

Angular velocity: exact `k2_so3_curve.py::SO3CurveStateDecoder._angular_motion` convention, `rel=RMB_t^T RMB_(t-1)`, `omega_body=-Log(rel)/dt`, first frame exactly zero, no future frame. Body omega is mapped through RMB to M frame and then to the PL root frame. The sampled AMASS overlay frame audit is poor; DIP and TotalCapture are strongly aligned. The sign/order is not switched to improve metrics.

Smoothing/state: causal order-2 Butterworth, 4 Hz at 60 Hz, first-sample steady-state initialization; `p0=pRL`, `v0=0`, `a_prev0=0`; full sequence initializes once.

Training/evaluation: AMASS 80 epochs -> DIP 40 epochs; no DIP translation supervision; no TotalCapture fine-tune. Best selection is integrated p + 0.25 state-v + 0.1 acceleration + normalized gravity angle proxy. Module reports p/v/a errors, per-leaf errors, gravity, dynamic residual, jerk, and 1/2/5/10-second/end drift.

Smoke: compile and all assertion tests passed; 2 sequences x 240 frames; all outputs finite; v head grad 2.52108, a head grad 0.008657, gravity head grad 0.143271. One-epoch candidate pRB L2 is 5.021169 cm versus official 2.494980 cm and is not evidence of improvement; candidate state-v L2 is 4.978820 cm/s and acceleration L2 is 417.787979 cm/s2.

Gate: run full pipeline only if same-cache DIP and TotalCapture candidate pRB L2 are both no worse than official PL and long-horizon outputs remain finite without severe drift.
