...
### EXP-20260712-PL-VA-State-V1

Question: can an opt-in PL-s1 backend predict root-frame leaf velocity and acceleration, integrate an explicit causal p/v/a state, and preserve the original 18D PL -> 63D IK1 interface without degrading same-cache PL position accuracy?

Protocol: AMASS official-route pretrain 80 epochs, DIP train/val fine-tune 40 epochs, DIP test and TotalCapture official test, no TotalCapture fine-tune, no DIP translation supervision. Full pipeline is conditional on candidate pRB L2 not exceeding official PL on both test sets.

Original contract: 84D `aRB+wRB+RRB+gR0`, 18D `pRB+gR1`, init `pRL+gR0`. New contract: 102D `aRB_raw+wRB_rmbdiff+RRB+gR0+aRB_smooth`; 33D raw `v_direct+a_leaf+gR1_raw`; beta=0.7 explicit trapezoidal state produces legacy 18D output.

RMB derivative: `rel_t=RMB_t^T RMB_(t-1)` and `omega_body=-axis_angle(rel_t)/dt`, copied from `k2_so3_curve.py::_angular_motion`; causal, first frame zero. Frame audit sampled three sequences each. Overall correlation/cosine: AMASS 0.223896/0.114226, DIP 0.986325/0.940574, TotalCapture 0.999153/0.987117. Keep the approved sign/order despite the AMASS mismatch.

Stage 0 command: `CUDA_VISIBLE_DEVICES=1 /home/lingfeng/bin/longrun -- bash custom_code/extra/scripts/run_pl_va_state_v1_smoke.sh`.

Stage 0 result: compile, unit tests, feature/state consistency, two-sequence cache, one-epoch forward/backward, and module eval passed. All outputs finite. Head gradient norms: v 2.521080, a 0.008657, gravity 0.143271. Candidate smoke pRB L2 5.021169 cm, direct-v L2 4.978933 cm/s, state-v L2 4.978820 cm/s, acceleration L2 417.787979 cm/s2, gravity 4.245395 deg. Official smoke pRB L2 2.494980 cm. This is smoke evidence only.

Artifacts: `data/experiments/pl_va_state_v1_20260712/smoke/summary.json`, `frame_audit.json`, cache manifest, train summary/checkpoints, eval metrics JSON and comparison CSV.
